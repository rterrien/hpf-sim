...
;; upsample_factor - use more pixels after array is read in, to increase fidelity of (riemann-sum) resampling. if -1, then do not break into upsample routine
...
pro hzpfsim_img, vrad, outfile, tellfile=tellfile, specfile = specfile, diagfile = diagfile, calfile = calfile, pixel_sampling = pixel_sampling, projection_type = projection_type, upsample_factor = upsample_factor, cal_upsample_factor = cal_upsample_factor, velshift_style = velshift_style, orders_lambdahigh = orders_lambdahigh, orders_lambdalow = orders_lambdalow, orders_gaps = orders_gaps, fiber_core_um = fiber_core_um, fiber_cladding_um = fiber_cladding_um, fiber_buffer_um = fiber_buffer_um, fiber_fractions = fiber_fractions, fiber_scale = fiber_scale, nfibers = nfibers, fiber_extra_sep_um = fiber_extra_sep_um, optical_model = optical_model, slitwidth_um = slitwidth_um, straight_orders = straight_orders, upfactor = upfactor
	...
	if n_elements(velshift_style) eq 0 then velshift_style = 'relativistic'
	...
	smooth_flag = projection_type eq 'simple' or projection_type eq '7_fibers'
	;smooth_flag=1
	...
	if keyword_set(specfile) then begin
		...
		ss = size(spec)
		if ss[0] ne 2 then stop
		if ss[2] gt 4 then spec = transpose(spec)
	endif else begin
	...
	if smooth_flag then begin
		fsms = dblarr(no,ng) ;smoothed array copies
		if calflag then calfsms = DBLARR(no,n_elements(calf))   ;;added 8/2/2012, CFB
		;loop over orders
		for i=0, no-1 do begin	
			print,'smoothing order ',i
	...
	case projection_type of
		...
		;'tram1':fiber_projection_mod, w, f, res, pixel_sampling, wlimg, specimg, calw=calw, calf=calfsms, diag_out = diag_out, fiber_fractions = fiber_fractions, orders_gaps = orders_gaps, orders_lambdalow = orders_lambdalow, orders_lambdahigh = orders_lambdahigh, warray = warray, fiber_scale = fiber_scale, fiber_core_um = fiber_core_um, fiber_cladding_um = fiber_cladding_um, fiber_buffer_um = fiber_buffer_um
		;'tram1': tram_projection3_mod, w, f, res, pixel_sampling, specimg, calw=calw, calf=calf, diag_out = diag_out, fiber_fractions = fiber_fractions, warray = warray, fiber_scale = fiber_scale, fiber_core_um = fiber_core_um, fiber_cladding_um = fiber_cladding_um, fiber_buffer_um = fiber_buffer_um, nfibers = nfibers, fiber_extra_sep_um = fiber_extra_sep_um, optical_model = optical_model, slitwidth_um = slitwidth_um, straight_orders = straight_orders, upfactor = upfactor
		else: stop
	endcase
...
pro smooth_res_ryan, res, midwl, wl, fl, fsm, diag_output = diag_output
	...
	cfl = convol(fl,kernel,/normalize,/center,/edge_zero)
	fsm = cfl
