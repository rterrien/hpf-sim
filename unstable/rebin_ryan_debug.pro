pro rebin_ryan_debug, pix, res, wl, fl, wlr, flr, minl, maxl, new_wl_in = new_wl_in
	...
	if n_elements(new_wl_in) ne 0 then begin
		newwl = new_wl_in
		npn = n_elements(newwl)
	endif
;;	if n_elements(new_wl_in) ne 0 then begin
;;		newwl = new_wl_in
;;		dwl = median(newwl[1:*] - newwl)
;;		nnew = ceil(maxl - max(new_wl_in))
;;		newwl2 = dindgen(nnew)*dwl + (max(new_wl_in) + dwl)
;;		newwl = [newwl,newwl2]
;;	endif
;;	stop
