...
pro tram_projection2, w, f, res, pixel_sampling, specimg, calw=calw, calf=calf, diag_out = diag_out, fiber_fractions = fiber_fractions, warray = warray, fiber_scale = fiber_scale, fiber_core_um = fiber_core_um, fiber_cladding_um = fiber_cladding_um, fiber_buffer_um = fiber_buffer_um, nfibers = nfibers, fiber_extra_sep_um = fiber_extra_sep_um, echellogram_file = echellogram_file, echellogram_wl_file = echellogram_wl_file, slitwidth_um = slitwidth_um
	...
	for i=0, norders-1 do begin
		...
		for j=0, (size(fs_base))[1]-1 do fs_base[j,i] = int_tabulated(wl_order_highs[left_inds[j]+1:right_inds[j]+1],fl_order_highs[left_inds[j]+1:right_inds[j]+1])
...
;;		print,'order ',i
		...
		tmp_conv_rebin = rebin(tmp_conv,x_size_d/2,y_size_d/2)
		;stop
