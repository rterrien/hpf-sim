...
pro tram_projection2c, w, f, res, pixel_sampling, specimg, calw=calw, calf=calf, diag_out = diag_out, fiber_fractions = fiber_fractions, warray = warray, fiber_scale = fiber_scale, fiber_core_um = fiber_core_um, fiber_cladding_um = fiber_cladding_um, fiber_buffer_um = fiber_buffer_um, nfibers = nfibers, fiber_extra_sep_um = fiber_extra_sep_um, echellogram_file = echellogram_file, echellogram_wl_file = echellogram_wl_file, slitwidth_um = slitwidth_um
	...
	for i=0, norders-1 do begin
		...
		wl_order_highs = dindgen(1d7)/1d7 * (max(ws_base_right[*,i]) - min(ws_base_left[*,i]) + 10.) + min(ws_base_left[*,i]) - 5.
		...
		warray[*,i] = ws_base[buffer/2 : 2047 + buffer/2,i]
		...
		y_size_kernel_big = 10d * y_size_kernel
		x_size_kernel_big = 10d * x_size_kernel
		kernel_big = dblarr(x_size_kernel_big,y_size_kernel_big)
		x_center_kernel_big = floor(x_size_kernel_big/2)
		tophat = kernel_big
		
		for j=0, nfibers-1 do begin
			ycenter = (j + 1.) * kernel_sep_pix * upfactor * 10d
			
			xth = rebin(dindgen(x_size_kernel_big),x_size_kernel_big,y_size_kernel_big) - x_center_kernel_big
			yth = rebin(1#dindgen(y_size_kernel_big),x_size_kernel_big,y_size_kernel_big) - ycenter
			rad = (kernel_size_pix * 10d)/2d
			radlocs = where(xth^2. + yth^2 le rad^2.,nradlocs)
			tophat[radlocs] = 1d
		endfor
		
		stop
		
		ee80 = psf_gaussian(npixel=[x_size_kernel_big,x_size_kernel_big],fwhm = 6.5*10d, centroid = [x_center_kernel_big,x_center_kernel_big],/double,/normalize)
		
		kernel_big = convol(tophat,ee80,/center,/edge_truncate)
		
		kernel = rebin(kernel_big,x_size_kernel,y_size_kernel)
...
;;			tk = psf_gaussian(npixel=[x_size_kernel,y_size_kernel],fwhm=kernel_size_pix*upfactor,centroid=[x_center_kernel,ycenter],/double,/normalize)/double(nfibers)
...
;;		print,'order ',i
		...
		tmp_conv = convol(tmp,kernel,/center,/edge_truncate)
		
		if upfactor ne 1 then tmp_conv_rebin = rebin(tmp_conv,x_size_d/2,y_size_d/2) else tmp_conv_rebin = tmp_conv
		;stop
		...
		if y1_det gt 2047 then begin
			...
			y1_det = 2047
			y1_subarr = y_size_d/upfactor - 1 - extra
		endif
