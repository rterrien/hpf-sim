...
pro tram_projection4_mod, w, f, res, pixel_sampling, specimg, calw=calw, calf=calf, diag_out = diag_out, fiber_fractions = fiber_fractions, warray = warray, fiber_scale = fiber_scale, fiber_core_um = fiber_core_um, fiber_cladding_um = fiber_cladding_um, fiber_buffer_um = fiber_buffer_um, nfibers = nfibers, fiber_extra_sep_um = fiber_extra_sep_um, optical_model = optical_model, slitwidth_um = slitwidth_um, orders_shape = orders_shape, upfactor = upfactor
...
;;		
		...
		y_size_kernel = (floor((nfibers + 1d) * kernel_sep_pix))*1d; + kernel_size_pix) + kernel_sep_pix)
		...
		for j=0, nfibers-1 do begin
			...
			tk = psf_gaussian(npixel=[x_size_kernel,y_size_kernel],fwhm=[kernel_size_pix,kernel_size_pix],centroid=[x_center_kernel,ycenter],/double,/normalize)/double(nfibers)
		...
		while y_size_d mod upfactor ne 0 do y_size_d += 1
		...
		;tmp = fs_rect
		tmp2 = rebin(tmp,x_size_d,y_size_d/upfactor)
		tmp_conv_rebin = convol(tmp2,kernel,/center,/normalize,/edge_zero)
		;stop
		;tmp_conv = tmp
