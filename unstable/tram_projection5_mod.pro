...
;;		
		...
		for j=0, nfibers-1 do begin
			ycenter = (j + 1.) * kernel_sep_pix * 1d
			tk = psf_gaussian(npixel=[x_size_kernel,y_size_kernel],fwhm=[kernel_size_pix*1d,kernel_size_pix*1d],centroid=[x_center_kernel,ycenter],/double,/normalize)/double(nfibers)
		...
		if upfactor ne 1 then tmp_rebin = rebin(tmp,x_size_d/upfactor,y_size_d/upfactor) else tmp_rebin = tmp
		tmp_conv_rebin = convol(tmp_rebin,kernel,/center,/normalize,/edge_zero)
