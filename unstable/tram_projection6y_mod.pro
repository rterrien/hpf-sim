...
;;	 ys = -1d * ys ;flip y around (stuart does this in his code)
	...
	if n_elements(upfactor) ne 0 then upfactor = double(upfactor)
	...
	fs_base = dblarr((2048 + buffer)*1d,norders)
	fs_base2 = fs_base
	
	xs_base = ((dindgen((2048+buffer)*1d) - (1024.+buffer/2d)*1d)/1d)
	ws_base = dblarr((2048 + buffer)*1d,norders)
	
	;find left and right limits of each pixel for binning the spectrum
	xs_base_left = xs_base - 0.5
	xs_base_right = xs_base + 0.5
	ws_base_left = dindgen((2048 + buffer) * 1d,norders)
	ws_base_right = dindgen((2048 + buffer) * 1d,norders)
	
	ys_base = dblarr((2048 + buffer) * 1d,norders)
	xs_base_2d = dblarr((2048 + buffer) * 1d,norders)
	...
	for i=0, norders-1 do begin
		...
		;subfls = (reform(f[i,*]))[li0[i]:li1[i]] ;fluxes for the order
		;subwls = reform(w[li0[i]:li1[i]]) ;wls for the order
		...
		wl_order_highs = dindgen(1d6)/1d6 * (max(ws_base_right[*,i]) - min(ws_base_left[*,i]) + 2.) + min(ws_base_left[*,i]) - 1.
		...
		;fl_order_highs = interpol(reform(f[i,*]),w*1d3,wl_order_highs);,/quadratic) ;when the smoothed one is being fed in
		fl_order_highs = interpol(reform(f),w*1d3,wl_order_highs);,/quadratic)

		;make sure the interpolation didn't extrapolate anywhere
		if noutlocs gt 0 then fl_order_highs[outlocs] = 0
		;stop
;;		ff1 = reform(fl_order_highs)
;;		ww1 = reform(wl_order_highs)
;;		smooth_res_ryan,50000.,median(ww1),ww1,ff1,ff2
;;		;fs_rect[*,100*upfactor] = ff2
;;		fl_order_highs = ff2
		...
		xs_rect = dblarr((2048+buffer)*1d,200*upfactor)
		ys_rect = dblarr((2048+buffer)*1d,200*upfactor)
		fs_rect = dblarr((2048+buffer)*1d,200*upfactor)
...
;;		ff1 = reform(fs_base[*,i])
;;		ww1 = reform(ws_base[*,i])
;;		smooth_res_ryan,50000.,median(ww1),ww1,ff1,ff2
;;		fs_rect[*,100*upfactor] = ff2							;GOOOOOOOD
;;		
		fs_rect[*,100*upfactor] = fs_base[*,i]
		
		
		
		;fs_rect[*,100] = [replicate(0,50),f[*,i],replicate(0,50)]
		;stop
		;subfls = (reform(f[i,*]))[li0[i]:li1[i]] ;fluxes for the order
		;subwls = reform(w[li0[i]:li1[i]]) ;wls for the order
		;stop
		
		;gg = where((wl_order_highs_left ge min(ws_base_left[*,i])) or (wl_order_highs_right le max(ws_base_right[*,i])))
		;subwls = wl_order_highs[gg] 
		;subfls = fl_order_highs[gg]
		...
		;warray[*,i] = wsub
		;fs_rect[*,100] = [replicate(0,50),fsub,replicate(0,50)]
		...
		x_size_kernel = (floor(3. * kernel_size_pix))*1d
		...
		for j=0, nfibers-1 do begin
			...
			tk = psf_gaussian(npixel=[x_size_kernel,y_size_kernel],fwhm=[kernel_size_pix*1d,kernel_size_pix*upfactor],centroid=[x_center_kernel,ycenter],/double,/normalize)/double(nfibers)
			
			;if there is a slit, apply it
			if n_elements(slitwidth_um) ne 0 then begin
				slitwidth_pix = slitwidth_um * fiber_scale * 1d
		...
		xs_rect_d = (xs_rect - xs_d_offset) * 1d
		...
		;stop
		;gx = x_grid_rect_d[1,0] - x_grid_rect_d[0,0]
		;gy = y_grid_rect_d[0,1] - y_grid_rect_d[0,0]
		;gx = (max(x_grid_rect_d[*,0]) - min(x_grid_rect_d[*,0]))/double(x_size_d)
		;gy = (max(y_grid_rect_d[0,*]) - min(y_grid_rect_d[0,*]))/double(y_size_d)
		...
		if upfactor ne 1 then tmp_conv_rebin = rebin(tmp_conv,st[0],st[1]/upfactor) else tmp_conv_rebin = tmp_conv
