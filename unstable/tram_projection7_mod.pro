...
;;	 ys = -1d * ys ;flip y around (stuart does this in his code)
	...
	xs_base = ((dindgen((2048+buffer)*1d) - (1024.+1d/2d)*1d)/1d)
	xs_base_high = ((dindgen((2048+buffer)*upfactor) - (1024.+upfactor/2d)*upfactor)/upfactor)
	...
	for i=0, norders-1 do begin
		...
		w1c = poly(xs_base_high,w1a)
		ws_base[*,i] = w1c
		dw1 = (ws_base[1:*,i] - ws_base[*,i])/2d
		dex = interpol(dw1,xs_base_high[0:-2],xs_base[-1])
		dw2 = [dw1,dex]
...
;;		fl_order_highs = ff2
		...
		xs_rect = dblarr((2048+buffer),200)
		ys_rect = dblarr((2048+buffer),200)
...
;;		
		...
		;if upfactor ne 1 then tmp_conv_rebin = rebin(tmp_conv,st[0]/upfactor,st[1]/upfactor) else tmp_conv_rebin = tmp_conv
		


		
		tmp = convol(fs_rect,kernel,/center,/normalize,/edge_zero)
		print,'convol done'
		
		st = size(tmp,/dimen)
		if upfactor ne 1 then tmp_rebin = rebin(tmp,st[0]/upfactor,st[1]/upfactor) else tmp_rebin = tmp
		
		tmp_warp = tri_surf(tmp_rebin,xs_rect_d,ys_rect_d,gs=[gx,gy])
		
		tmp_conv_rebin = tmp_warp
		print,'trisurf done'
		;stop
		;stop
		;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
